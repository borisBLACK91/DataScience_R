...
#Imagine you draw two balls from a box containing colored balls. 
#You either replace the first ball before you draw the second or you 
#leave the first ball out of the box when you draw the second ball.
...
#You do replace the first ball before drawing the next.(GOOD ANSWER!)
...
#first draw. Using the variable p_yellow, calculate the probability of choosing 
#a yellow ball on the sixth draw.
cyan <- 3
magenta <- 5
yellow <- 7
...
#Calculate the probability that the Celtics win at least one game in the first four
#games of the series.
# Assign the variable `p_cavs_win4` as the probability that the Cavs will win the 
...
#simulated_games. Then, use the any function to indicate whether the four-game series 
#contains at least one win for the Celtics. Perform these operations in two separate steps.
#Use the mean function on celtic_wins to find the proportion of simulations that contain at
#least one win for the Celtics out of four games.
...
#for B iterations: (1) generating a random four-game series `simulated_games` using 
#the example code, then (2) determining whether the simulated series contains at least
#one win for the Celtics.
wins <- "win"
celtic_wins <- replicate(B, {simulated_games <- sample(c("lose","win"), 4, replace = TRUE, prob = c(0.6, 0.4))
wins %in% simulated_games 
})
# Calculate the frequency out of B iterations that the Celtics won at least
#one game. Print your answer to the console.
mean(celtic_wins)
