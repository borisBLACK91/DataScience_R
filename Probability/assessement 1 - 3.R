...
#Use the rep function to create a list of n games, where each game 
#consists of list(outcomes).
#Use the expand.grid function to create a data frame containing all the combinations of
#possible outcomes of the remaining games.
...
#Use the mean function to calculate the proportion of outcomes that result in 
#the Cavs winning the series and print your answer to the console.
...
# Create a vector named 'results' that indicates whether each
#row in the data frame 'possibilities' contains enough wins for the
#Cavs to win the series.
results <- rowSums(possibilities)>=4

# Calculate the proportion of 'results' 
#in which the Cavs win the series. Print the outcome to the console.
mean(results)
...
# Create an object called `results` that replicates for `B` 
#iterations a simulated series and determines whether that 
#series contains at least four wins for the Cavs.
n <- 6

results <- replicate(B,{
  series <- sample(c(0,1), n, replace = TRUE)
  sum(series)>=4
})
mean(results)
...
#Instructions
#Use the function sapply to compute the probability, call it Pr of winning
#for p <- seq(0.5, 0.95, 0.025).
#Then plot the result plot(p, Pr).
# Let's assign the variable 'p' as the vector of probabilities that team 
#A will win.
...
# Apply the 'prob_win' function across the vector of probabilities that
#team A will win to determine the probability that team B will win.
#Call this object 'Pr'.
...
#that team A wins fixed at p <- 0.75 and compute the probability 
#for different series lengths. For example, wins in best of 1 game,
#3 games, 5 games, and so on through a series that lasts 25 games.
#Instructions
#Use the seq function to generate a list of odd numbers ranging from 1 to 25.
#Use the function sapply to compute the probability, call it Pr, of winning 
#during series of different lengths.
