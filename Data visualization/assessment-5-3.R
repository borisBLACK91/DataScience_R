...
#The sample code given creates a tile plot showing the
#rate of measles cases per population. We are going to
#modify the tile plot to look at smallpox cases instead.
#Instructions
#Modify the tile plot to show the rate of smallpox cases 
#instead of measles cases.
#Exclude years in which cases were reported in fewer than 
#10 weeks from the plot.
...
#The sample code given creates a time series plot showing 
#the rate of measles cases per population by state.
#We are going to again modify this plot to look at smallpox cases instead.
#Instructions
#Modify the sample code for the time series plot to plot data
#for smallpox instead of for measles.
...
#Instructions
#For the state of California, make a time series plot showing
#rates for all diseases.
#Include only years with 10 or more weeks reporting.
#Use a different color for each disease.
#Include your aes function inside of ggplot rather than inside 
#your geom layer.
...
#diseases in the United States. For this exercise, we have provided 
#less sample code - you can take a look at the previous exercise to 
#get you started.
#Instructions
#Compute the US rate by using summarize to sum over states. 
#Call the variable rate.
#The US rate for each disease will be the total number 
#of cases divided by the total population.
...
us_contagious_diseases %>% filter(!is.na(population)) %>% 
  ...
  ggplot(aes(year, rate, color = disease)) + 
  geom_line() + 
  ggtitle("US avg deasases rate over the years")